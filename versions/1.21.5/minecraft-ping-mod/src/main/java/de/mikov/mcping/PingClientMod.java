...
public class PingClientMod implements ClientModInitializer {
    ...
    private static final String KEY_CATEGORY_TRANSLATION = "key.categories.simplemultiplayerping";
    ...
    private Vec3d savedPingTarget = null;
    ...
    public void onInitializeClient() {
        ...
        pingKey = KeyBindingHelper.registerKeyBinding(createKeyBindingCompat(
            "key.mcping.ping",
            InputUtil.Type.MOUSE,
            GLFW.GLFW_MOUSE_BUTTON_MIDDLE
        ));

        partyMenuKey = KeyBindingHelper.registerKeyBinding(createKeyBindingCompat(
            "key.mcping.party",
    ...
    public boolean isPingKeyDown() {
        ...
        InputUtil.Key bound = resolveBoundKey(pingKey);
        if (bound == null) {
            return false;
        }
    ...
    private void onClientTick(MinecraftClient client) {
        ...
        boolean pressed = isKeyPhysicallyPressed(pingKey);
        ...
        if (pressed && !pingKeyDown) {
            ...
            savedPingTarget = resolveTargetPos(client);
        }
    ...
    private void trySendPingAtCrosshair(MinecraftClient client, PingType pingType) {
        if (client.player == null || client.world == null) {
            LOGGER.warn("Ping aborted: player={} world={}", client.player, client.world);
            return;
        }

        if (!partyState.inParty()) {
            client.player.sendMessage(Text.literal("Open the party menu with P and join a party first."), true);
            LOGGER.warn("Ping aborted: not in party");
            return;
        }

        Vec3d pos = savedPingTarget;
        if (pos == null) {
            pos = resolveTargetPos(client);
        }
        savedPingTarget = null;

        if (pos == null) {
            client.player.sendMessage(Text.literal("No block targeted."), true);
            LOGGER.warn("Ping aborted: no block targeted");
            return;
        }
    ...
    private static Vec3d resolveTargetPos(MinecraftClient client) {
        // Primary: use Minecraft's own crosshair target (most reliable)
        try {
        ...
        // Fallback: manual raycast from player
        try {
            if (client.player != null) {
    ...
    private static KeyBinding createKeyBindingCompat(String translationKey, InputUtil.Type inputType, int code) {
        try {
            ...
            Object categoryIdentifier = identifierOf.invoke(null, "simplemultiplayerping", "general");

            Class<?> keyBindingCategoryClass = Class.forName("net.minecraft.client.option.KeyBinding$Category");
            Method categoryCreate = keyBindingCategoryClass.getMethod("create", identifierClass);
            Object category = categoryCreate.invoke(null, categoryIdentifier);

            Constructor<?> modernConstructor = KeyBinding.class.getConstructor(
                    String.class,
                    InputUtil.Type.class,
                    int.class,
                    keyBindingCategoryClass
            );
            return (KeyBinding) modernConstructor.newInstance(translationKey, inputType, code, category);
        } catch (ReflectiveOperationException ignored) {
        }

        try {
            Constructor<KeyBinding> legacyConstructor = KeyBinding.class.getConstructor(
                    String.class,
                    InputUtil.Type.class,
                    int.class,
                    String.class
            );
            return legacyConstructor.newInstance(translationKey, inputType, code, KEY_CATEGORY_TRANSLATION);
        } catch (ReflectiveOperationException ex) {
            throw new IllegalStateException("Unable to create compatible key binding", ex);
        }
    }

    private static boolean isKeyPhysicallyPressed(KeyBinding keyBinding) {
        MinecraftClient client = MinecraftClient.getInstance();
        if (client == null || client.getWindow() == null) {
            return false;
        }
        long handle = client.getWindow().getHandle();
        InputUtil.Key bound = resolveBoundKey(keyBinding);
        if (bound == null) {
            return false;
        }
        if (bound.getCategory() == InputUtil.Type.MOUSE) {
    ...
    private static InputUtil.Key resolveBoundKey(KeyBinding keyBinding) {
        try {
        ...
        try {
            Method getBoundKey = KeyBinding.class.getMethod("getBoundKey");
            Object key = getBoundKey.invoke(keyBinding);
            if (key instanceof InputUtil.Key inputKey) {
                return inputKey;
            }
        } catch (ReflectiveOperationException ignored) {
        }
